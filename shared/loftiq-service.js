...
// Lofty Academy training videos: title+description+category only (no transcript is stored per
// video) - enough for Lotti to point someone to the right video by name, not recite its
// narration. Same reach as searchLoftIqAlignments/searchLoftIqKnowledge above.
export const searchLoftIqLearningMaterials=(query)=>api(`/api/my/loftiq/learning-search?q=${encodeURIComponent(query)}`);
...
export function buildLoftIqPrompt({context,priorMessages,question,helpArticles,alignments,trainingVideos}){
  ...
  const hasGeneralKnowledge=(helpArticles&&helpArticles.length)||(alignments&&alignments.length)||(trainingVideos&&trainingVideos.length);
...
${trainingVideos&&trainingVideos.length?'\nTRAINING_VIDEOS below are Lofty Academy catalog entries (title + short description only - you do NOT have the video\'s actual narrated content/transcript). If one is relevant, tell the employee its exact title and that it\'s in Lofty Academy on the portal so they can watch it - do not claim to know the specific steps it walks through beyond what the description already says, and never invent narration.\n':''}${hasGeneralKnowledge?'\nGENERAL_KNOWLEDGE below (Lofty product help articles, company Alignment SOPs/policies, and Lotti Knowledge items) is a SEPARATE thing from the employee\'s own data above - it\'s general reference material, not anything personal to them. Alignment/Knowledge entries are the portal\'s own official, admin-authored SOPs - treat them as authoritative for "what\'s the process/policy for Y" questions.\n\nSTRICT RULE FOR MULTIPLE ENTRIES: when GENERAL_KNOWLEDGE has more than one entry, go through them ONE BY ONE and judge each on its own merit - never just answer from whichever single entry happens to share the most words with the question, and never stop reading after the first plausible-looking match. Match on MEANING, not literal word overlap: an entry that never says the question\'s exact word can still be the right one (e.g. a category described as "account status, reactivation, deactivation" covers a question about "cancellation," since a cancellation IS a deactivation), while an entry that repeats the question\'s exact word a lot can still be the wrong one if it\'s answering a different kind of question about that same topic.\n\nFIRST figure out what TYPE of answer the question actually wants, since a topic can have more than one kind of entry about it: a CONTACT/ROUTING question ("who do I contact," "which email," "who do I send this to") wants a specific person/email/team; a PROCESS/POLICY question ("what do I tell a customer," "how do I handle X," "what\'s not allowed") wants handling rules or a procedure. If one entry is clearly built as a contact directory (categorized list of situations each mapped to a specific contact) and its category covers the question\'s topic - even via a related term like the deactivation/cancellation example above - that entry alone is the complete, sufficient answer to a CONTACT question; do not also pull in a separate entry\'s internal handling/discussion rules or ticket-routing steps just because it mentions the same topic word, since that entry is answering the PROCESS question, not the one asked. Only combine multiple entries when the question itself is actually asking about more than one of those things at once (e.g. "who do I contact and what am I allowed to tell them" genuinely needs both). If nothing in there actually answers the question, say so rather than stretching an unrelated article to fit.':''}
