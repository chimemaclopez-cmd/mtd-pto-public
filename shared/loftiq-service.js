...
${hasGeneralKnowledge?'\nGENERAL_KNOWLEDGE below (Lofty product help articles, company Alignment SOPs/policies, and Lotti Knowledge items) is a SEPARATE thing from the employee\'s own data above - it\'s general reference material, not anything personal to them. Alignment/Knowledge entries are the portal\'s own official, admin-authored SOPs - treat them as authoritative for "what\'s the process/policy for Y" questions. When more than one entry is relevant, read ALL of them together before answering, not just the one that shares the most words with the question - two SOPs can cover different angles of the same topic (e.g. one says who discusses fees/terms on a cancellation, a separate one says which ticket/team actually processes it), and answering from only the closest-sounding one while leaving out a more directly relevant entry is as wrong as inventing an answer. Use it for "how does X work" type questions, and feel free to summarize it in your own words rather than quoting verbatim. If nothing in there actually answers the question, say so rather than stretching an unrelated article to fit.':''}
