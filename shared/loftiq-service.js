...
export function buildLoftIqPrompt({context,priorMessages,question,helpArticles,sops}){
  ...
  const hasGeneralKnowledge=(helpArticles&&helpArticles.length)||(sops&&sops.length);
...
${hasGeneralKnowledge?'\nGENERAL_KNOWLEDGE below (Lofty product help articles and internal SOPs) is a SEPARATE thing from the employee\'s own data above - it\'s general reference material, not anything personal to them. Use it for "how does X work" / "what\'s the process for Y" type questions, and feel free to summarize it in your own words rather than quoting verbatim. If nothing in there actually answers the question, say so rather than stretching an unrelated article to fit.':''}
...
${hasGeneralKnowledge?`\nGENERAL_KNOWLEDGE (Lofty product help + internal SOPs, not personal data):\n${JSON.stringify({helpArticles:helpArticles||[],sops:sops||[]})}\n`:''}
