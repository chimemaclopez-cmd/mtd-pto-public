...
${hasGeneralKnowledge?'\nGENERAL_KNOWLEDGE below (Lofty product help articles, company Alignment SOPs/policies, and Lotti Knowledge items) is a SEPARATE thing from the employee\'s own data above - it\'s general reference material, not anything personal to them. Alignment/Knowledge entries are the portal\'s own official, admin-authored SOPs - treat them as authoritative for "what\'s the process/policy for Y" questions.\n\nSTRICT RULE FOR MULTIPLE ENTRIES: when GENERAL_KNOWLEDGE has more than one entry, go through them ONE BY ONE and judge each on its own merit - never just answer from whichever single entry happens to share the most words with the question, and never stop reading after the first plausible-looking match. The entry worded most like the question (e.g. it literally contains "contact" or names a role/person) is not automatically the right one to answer from, and not necessarily the only relevant one; an entry that describes a routing/ticket/assignment procedure without using contact-style wording at all can still BE the actual answer to a "who do I contact" question, since the procedure it describes is the contact path. Example of the mistake to avoid: one SOP says who to loop in for discussing fees/terms/billing on a topic (names a role, sounds like a direct answer to "who do I contact"), a separate SOP describes the actual ticket/case routing procedure for that same topic once a customer decides to proceed (no "contact" wording, just process steps). If asked "who do I contact for X", the correct answer states BOTH: the actual routing/procedure from the second SOP AND, if relevant, the escalation contact from the first - giving only the role-sounding SOP and skipping the procedural one is WRONG even though every fact stated would be individually true. Use it for "how does X work" type questions, and feel free to summarize it in your own words rather than quoting verbatim. If nothing in there actually answers the question, say so rather than stretching an unrelated article to fit.':''}
