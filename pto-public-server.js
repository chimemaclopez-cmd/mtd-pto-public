...
// Team leads correcting a direct report's PERMANENT weekly pattern (not just one day) queue
// their weekday-template edit here instead - same no-local-filesystem-access reasoning as
// SCHEDULE_OVERRIDE_UPDATES_KEY above. zendesk-proxy.js's syncScheduleTemplateUpdatesFromCloud()
// drains this queue, editing the employee's active schedules.json record in place (preserving
// the prior state in data.history first), not creating a new effective-dated period.
...
const STATIC_SHARED_BINARY = new Set(['img/lofty-logo.png', 'img/moatable-logo.png', 'img/icon-192.png', 'img/icon-512.png', 'img/icon-512-maskable.png', 'img/apple-touch-icon.png', 'img/csat-banner.png', 'img/learning-thumb-zendesk-sop.png', 'img/learning-thumb-intro-lofty.png', 'img/learning-thumb-jira-training.png', 'img/learning-thumb-ess-training.png', 'img/learning-thumb-tsr-escalation.png', 'img/learning-thumb-toolkit-remove-listing-photos.png', 'img/learning-thumb-toolkit-remove-sold-listing.png', 'img/learning-thumb-toolkit-image-sync.png', 'img/learning-thumb-toolkit-landing-page.png', 'img/learning-thumb-toolkit-blog-editor.png', 'img/learning-thumb-toolkit-copy-website.png', 'img/learning-thumb-toolkit-domain-transfer.png', 'img/learning-thumb-lofty-front.png', 'img/learning-thumb-fbe-billing-basics.png', 'vendor/pptxgen.bundle.js', 'vendor/jspdf.umd.min.js']);
...
async function computeStatusWall() {
  ...
  for (const emp of roster) {
    ...
    } else if ((liveOnline || selfReportedQueue) && emp.primaryChannel !== 'Email / Chat') {
      // "Avail" means "waiting for the next call" - meaningful for phone-capable reps (Voice,
      // Multi-channel), but a non-voice Email/Chat rep with no active ticket isn't "available"
      // in that sense, they're just between tickets - fall through so Off Queue/Not Reported
      // reflects that instead of a misleading green Avail pill.
      statusLabel = 'Avail';
      statusCode = 'AVAIL';
...
const DEFAULT_GUIDES = {
  ...
  steps: {
    ...
    'email-owned-setup': { id: 'email-owned-setup', text: 'Customer-owned domain setup', resolution: "Never invent DNS values - copy DKIM selectors/targets, verification tokens, and SPF/DMARC values from the account's own current setup screen (Settings > Email Settings/Sending Email Domain > Add/Configure/Reconfigure sending domain), not from a generic example. Add every DKIM CNAME shown - a partial match or a missing record counts as incomplete (one real ticket needed 3 CNAMEs; only 2 confirmed left it stuck). For SPF, merge into the existing record with 'include:amazonses.com' - never publish a second 'v=spf1' TXT record at the same hostname. Preserve DMARC/MX/A records unless the approved design explicitly changes them: don't touch MX just to enable sending (use a dedicated sending subdomain instead if the primary domain already routes mail elsewhere), and don't add an apex A record for email auth alone - confirm what any existing A record is actually for first (it may be the customer's live website). After DNS changes, use Lofty's Check DNS/Verify action, then send a controlled test from the exact intended From address to both a Gmail/Google Workspace mailbox and a Microsoft mailbox before closing out." },
    'email-backend-review': { id: 'email-backend-review', text: 'Backend/domain-binding review', resolution: "This is a backend/domain-binding issue - skip customer-side DNS changes and inspect platform binding, SES identity, renewal status, and account mapping instead; don't ask the customer to edit records they don't control. Applies whether the domain is Lofty-provided (previously working or not) or customer-owned with every DNS record confirmed correct - compare each returned DKIM target exactly via dig, count SPF records, check propagation from multiple public resolvers - but still stuck after the normal propagation window." },
    ...
    'email-c-valid-owner': { id: 'email-c-valid-owner', text: 'Current owner is valid', resolution: 'Confirm whether duplicate use is actually intended. If not, explain the ownership constraint to the customer and pause here - do not detach an active domain or let it be reused in parallel without ownership approval. Proceed only once the desired final mapping (domain, account, user, instance) is confirmed.' },
    'email-ownership-transfer': { id: 'email-ownership-transfer', text: 'Ownership transfer/release review', resolution: "Escalate immediately for a safe detach or reassignment - identify the owning account/seat/office/instance, confirm authorization, and route to the team that can release or transfer without disrupting the existing instance. Always escalate right away for a prior-instance domain, a branded domain moving between team/private instances, or a domain tied to the wrong org/seat." },
    ...
    'email-e-bounce': { id: 'email-e-bounce', text: 'Bounce/deliverability triage', resolution: 'An outbound log confirms the send attempt is real, so this is deliverability triage, not sending-domain setup. Review bounce, suppression, and recipient-specific conditions; if a verification email specifically was not received, also check recipient spelling and mailbox rules, and remove any suppression only through the approved process before retrying. A sender-domain warning can coexist with successful sending to some recipients, so the visible warning may not fully explain this specific symptom.' },
    'email-e-inbound': { id: 'email-e-inbound', text: 'Inbound/MX routing', resolution: "Outbound working but replies never arriving is inbound mailbox/MX-routing, not sending-domain verification - investigate MX, mailbox existence, inbound routing/parsing, and timeline ingestion separately. Do not conclude that outbound DKIM/SPF verification proves inbound routing works." },
  ...
  try {
    ...
    if (parsed.pathname === '/api/my/loftiq/context' && req.method === 'GET') {
      ...
      if (assignedMembers.length) {
        ...
        // csatGoodNeededFor95/lcrShortCallsNeededBelow5 mirror goodNeededFor95/
        // shortCallsNeededBelow5 in pto-public.html exactly (the Rep Goal Tracker on the KPI
        // Scores tab) - a real question ("how many good ratings does X need for 95%") was
        ...
        const teamMembers = teamRows.map(r => {
          ...
          const csatGoodNeededFor95 = surveys ? Math.max(0, 19 * bad - good) : null;
          ...
          return {
            ...
            csatGood: good, csatBad: bad, csatRate: surveys ? Math.round((good / surveys) * 1000) / 10 : null, csatGoodNeededFor95,
    ...
    if (parsed.pathname === '/api/leadership/operational-notes' && req.method === 'POST') {
      ...
      const item = {
        ...
        otherKpiDetail: String(body.otherKpiDetail || '').trim(),
    ...
    if (operationalNoteMatch && (req.method === 'PUT' || req.method === 'DELETE')) {
      ...
      list[index] = {
        ...
        otherKpiDetail: body.otherKpiDetail !== undefined ? String(body.otherKpiDetail || '').trim() : current.otherKpiDetail,
    ...
    if (parsed.pathname === '/api/my/team-schedule-template' && req.method === 'POST') {
      const body = await readJsonBody(req);
      const email = ptoLogic.cleanEmail(body.employeeEmail || '');
      ...
      const scopedMembers = scopedTeamMembers(roster, identity, session, session.employeeName, false, true);
      if (!scopedMembers.some(x => ptoLogic.cleanEmail(x.employeeEmail) === email)) return json(res, 403, { ok: false, error: 'Not your direct report.' });
      if (!['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].includes(weekday)) return json(res, 400, { ok: false, error: 'A valid weekday is required.' });
      const off = Boolean(body.off);
      const assignments = {};
      if (!off) {
        for (const [time, rawActivityId] of Object.entries(body.assignments || {})) {
      ...
      const now = new Date().toISOString();
      const schedules = await loadScheduleSnapshot();
      const record = (schedules.schedules || []).filter(x => ptoLogic.cleanEmail(x.employeeEmail) === email && x.active).sort((a, b) => b.effectiveFrom.localeCompare(a.effectiveFrom))[0];
      if (!record) return json(res, 400, { ok: false, error: 'This team member has no active schedule record yet - ask an admin to create one first.' });
      const queuedTemplate = { employeeEmail: email, weekday, off, shiftStartEastern, shiftEndEastern, assignments, exactActivities, updatedBy: identity, updatedAt: now };
      const pending = await cloudStore.kvGetJson(SCHEDULE_TEMPLATE_UPDATES_KEY, []);
      pending.push(queuedTemplate);
      await cloudStore.kvSetJson(SCHEDULE_TEMPLATE_UPDATES_KEY, pending);
      // Optimistic local reflection, same reasoning as the day-override POST above.
      record.weekly[weekday] = { off, shiftStartEastern, shiftEndEastern, overnight: record.weekly?.[weekday]?.overnight || false, assignments, defaultAssignment: record.weekly?.[weekday]?.defaultAssignment || '' };
      if (!off) {
        record.exactActivities = record.exactActivities || {};
        record.exactActivities[weekday] = exactActivities;
      } else if (record.exactActivities) {
        delete record.exactActivities[weekday];
      }
      await cloudStore.kvSetJson('mtdkpi:snapshot:schedules', schedules);
      snapshotCache.set('mtdkpi:snapshot:schedules', { value: schedules, at: Date.now() });
      return json(res, 200, { ok: true });
    ...
    // evaluation covers rather than the full 1..evalMonth trend - lets either the rep (viewing
    // their own evaluation) or their team lead (viewing a report's, e.g. from Team Evaluations)
    // see the same KPI/coaching context the evaluation was written against, right on the
    // sign-and-review / preview screen. Field shapes intentionally match zendesk-proxy.js's
    // buildEvaluationAppendixData() (kpiRows/coachingLogs) so the client can share one render
    // function with the exported-PDF appendix.
    if (parsed.pathname === '/api/my/evaluation-appendix' && req.method === 'GET') {
      const evaluationId = parsed.searchParams.get('evaluationId') || '';
      const evaluations = await loadEvaluations();
      const record = (evaluations.records || []).find(r => r.evaluationId === evaluationId);
      if (!record) return json(res, 400, { ok: false, error: 'Evaluation not found.' });
      const recordEmail = ptoLogic.cleanEmail(record.employeeEmail);
      const roster = await loadRosterSnapshot();
      const isSelf = recordEmail === identity;
      const isTeamLead = !isSelf && scopedTeamMembers(roster, identity, session, session.employeeName).some(m => ptoLogic.cleanEmail(m.employeeEmail) === recordEmail);
      ...
      const productivityRaw = productivityCount != null && workedDays ? Math.round(productivityCount / workedDays) : null;
      const productivityTier = productivityRaw != null ? scoreProductivityTierPortal(productivityRaw, member.kpiType) : null;
      const csatRate = metrics && !metrics.error && (metrics.csatGood + metrics.csatBad) > 0 ? metrics.csatGood / (metrics.csatGood + metrics.csatBad) * 100 : null;
      const csatTier = scoreCsatTierPortal(csatRate);
      const compliancePercent = complianceStore?.[recordEmail]?.[periodNumber]?.percent ?? null;
      const attendancePercent = attendanceRange?.attendancePercentage ?? null;
      const weighted = {
        productivity: productivityTier == null ? null : productivityTier * 0.40,
        csat: csatTier == null ? null : csatTier * 0.30,
      ...
      const knownWeights = Object.values(weighted);
      const totalScore = knownWeights.some(w => w != null) ? knownWeights.reduce((sum, w) => sum + (w || 0), 0) : null;
      const kpiRows = [{
        periodNumber, periodStart, periodEnd: periodEndInclusive, workedDays,
        productivityRaw, productivityTotal: productivityCount, productivityKind: selectedKind, productivityTier,
        csatRaw: csatRate, csatTier, complianceRaw: compliancePercent, attendanceRaw: attendancePercent,
        totalScore, totalScoreProvisional: knownWeights.some(w => w == null)
      }];
      const coachingLogs = (coachingData.records || [])
